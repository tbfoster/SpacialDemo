...
import mri.v3ds.PGraphicsOpenGL;
...
//**************************************************************************
    ...
    public void loadScene(String filename)
    {
        ...
        for (int m = 0; m < _scene.materials(); m++)
        {
            ...
            mmat.texId = -1;
            mmat.name = mat.name();
            mmat.ambient = new float[]
                    {
                        mat.ambient().red(), mat.ambient().green(), mat.ambient().blue(), 1
                    };
            ...
            mmat.specular = new float[]
                    {
                        mat.specular().red(), mat.specular().green(), mat.specular().blue(), 1
                    };
            ...
            for (int fm = 0; fm < m.faceMats(); fm++)
            {
                ...
                if (_useMaterials)
                {
                    ...
                    if (mat != null)
                    {
                        ...
                        _gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_SHININESS, new float[]
                                {
                                    32
                                }, 0);
                ...
                } else
                {
                    _gl.glEnable(GL.GL_COLOR_MATERIAL);
                    //_gl.glMaterialfv( GL.GL_FRONT_AND_BACK, GL.GL_AMBIENT, mat.ambient, 0 );
                    _gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_DIFFUSE, new float[]
                            {
                                _r, _g, _b, _a
                            }, 0);
                    _gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_SHININESS, new float[]
                    ...
                    _gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_SPECULAR, new float[]
                            {
                                1, 1, 1, 1
                            }, 0);
                    _gl.glBindTexture(GL.GL_TEXTURE_2D, 0);
                ...
                for (int fi = 0; fi < fmat.faces(); fi++)
                {
                    ...
                    if (mat != null)
                    {
                        _gl.glColor4f(mat.diffuse[0], mat.diffuse[1], mat.diffuse[2], mat.diffuse[3]);
                    } else
                    {
                        _gl.glColor4f(_r, _g, _b, _a); //1, 1, 1, 1 );
                    }
                    _gl.glNormal3f(n0.x, n0.y, n0.z);
                    if (m.texCoords() > 0)
                    {
                        _gl.glTexCoord2f(uv0.x, uv0.y);
                    }
    ...
    public void computeNormals()
    {
        ...
        for (int i = 0; i < _scene.meshes(); i++)
        {
            ...
            for (int vi = 0; vi < m.vertices(); vi++)
            {
                ...
                if (FLIPYZ)
                {
                    _meshes[i].vertices[vi] = new PVector(p.X, -p.Z, p.Y);
                } else
                {
                    _meshes[i].vertices[vi] = new PVector(p.X, p.Y, p.Z);
                }
                ...
                if (m.texCoords() > 0)
                {
                    if (FLIPV)
                    {
                        _meshes[i].texCoords[vi] = new PVector(tc.U, 1.0f - tc.V, 0);
                    } else
                    {
                        _meshes[i].texCoords[vi] = new PVector(tc.U, tc.V, 0);
                    }
                }
            }
